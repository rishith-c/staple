...
STAPLE_HOST_PORT="${STAPLE_HOST_PORT:-3100}"
STAPLE_HOST_FROM_CONTAINER="${STAPLE_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.staple-openclaw.override.yml"
...
detect_staple_base_url() {
  ...
  for candidate in "$STAPLE_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${STAPLE_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${STAPLE_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
staple_base_url="$(detect_staple_base_url || true)"
...
Staple URL for OpenClaw container:
EOF
  if [[ -n "$staple_base_url" ]]; then
    cat <<EOF
  $staple_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${STAPLE_HOST_PORT}
  (Do not use http://127.0.0.1:${STAPLE_HOST_PORT} inside the container.)
  If Staple rejects the host, run on host machine:
    pnpm stapleai allowed-hostname host.docker.internal
  Then restart Staple and re-run this script.
EOF
  fi
  cat <<EOF
