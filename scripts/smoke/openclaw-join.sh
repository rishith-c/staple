...
COMPANY_ID="${COMPANY_ID:-${STAPLE_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-staple-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-staple-openclaw-smoke}"
...
Provide one of:
  STAPLE_AUTH_HEADER=\"Bearer <board-token>\"
  STAPLE_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Staple OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.staple.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
