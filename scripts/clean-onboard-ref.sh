...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Staple data dir to use
  STAPLE_HOST=127.0.0.1    Host passed to the onboarded server
  STAPLE_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/staple-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/staple-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export STAPLE_HOST="${STAPLE_HOST:-127.0.0.1}"
export STAPLE_PORT="${STAPLE_PORT:-3100}"
export STAPLE_OPEN_ON_LISTEN="${STAPLE_OPEN_ON_LISTEN:-false}"
...
echo "STAPLE_HOST: $STAPLE_HOST"
echo "STAPLE_PORT: $STAPLE_PORT"
...
env \
  ...
  HOST="$STAPLE_HOST" \
  PORT="$STAPLE_PORT" \
  STAPLE_OPEN_ON_LISTEN="$STAPLE_OPEN_ON_LISTEN" \
  pnpm stapleai onboard --yes --data-dir "$PC_DATA"
