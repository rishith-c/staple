...
const JWT_SECRET_ENV_KEY = "STAPLE_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Staple CLI commands",
...
export function resolveStapleEnvFile(configPath?: string): string {
...
export function loadStapleEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeStapleEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readStapleEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeStapleEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeStapleEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readStapleEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeStapleEnvEntries(next, filePath);
  return next;
}
