...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Staple API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Staple server is not running, the configured URL is wrong, or the request is being blocked before it reaches Staple.",
    "",
    "Try:",
    "- Start Staple with `pnpm dev` or `pnpm stapleai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Staple is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`STAPLE_API_URL\`.`,
  );
  return lines.join("\n");
}
