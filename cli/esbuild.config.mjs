/**
 * esbuild configuration for building the stapleai CLI for npm.
 *
 * Bundles all workspace packages (@stapleai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @stapleai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@stapleai/")) {
      externals.add(name);
    }
  }
