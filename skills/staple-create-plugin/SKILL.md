---
name: staple-create-plugin
description: >
  Create new Staple plugins with the current alpha SDK/runtime. Use when
...
# Create a Staple Plugin

Use this skill when the task is to create, scaffold, or document a Staple plugin.
...
node packages/plugins/create-staple-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Staple repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.staple-sdk/`:

```bash
pnpm --filter @stapleai/create-staple-plugin build
node packages/plugins/create-staple-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Staple from an absolute local path during development
