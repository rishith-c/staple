...
Staple companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .staple.yaml     # Adapter config, env inputs, routines
```
...
- **.staple.yaml** holds Staple-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
stapleai company export <company-id> --out ./my-export
```
...
stapleai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
stapleai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
stapleai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.staple.yaml`
...
```sh
# From a local folder
stapleai company import ./my-export

# From a GitHub URL
stapleai company import https://github.com/org/repo

# From a GitHub subfolder
stapleai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
stapleai company import org/repo
stapleai company import org/repo/companies/acme
```
...
If `--target` is not specified, Staple infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
stapleai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
stapleai company import org/company-templates/engineering-team \
  --target new \
...
```sh
stapleai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
stapleai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
stapleai company import ./package \
  --target new \
  --yes \
  --json
```
...
Staple supports several GitHub URL formats:
