...
Staple is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── staple/               # Core Staple skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Staple env vars and a prompt
4. **Agent work** — The agent calls Staple's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Staple and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `stapleai run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — Staple orchestrates agents; it doesn't run them
