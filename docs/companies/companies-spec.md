...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Staple.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.staple.yaml`
...
- Staple must not require extra top-level fields for skill validity
- Staple-specific extensions must live under `metadata.staple` or `metadata.sources`
...
For Staple, the preferred fidelity extension is:
...
- issue/task Staple-only metadata
...
- Staple tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Staple shape:
...
Additional rules for Staple exporters:
...
- omit empty and default-valued Staple fields when possible
...
- Staple exporters should emit `.staple.yaml` as a sidecar by default
...
Staple can map this spec to its runtime model like this:
...
- Staple extension:
  - `.staple.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Staple-specific fidelity

Inline Staple-only metadata that must live inside a shared markdown file should use:

- `metadata.staple`

That keeps the base format broader than Staple.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Staple.

## 20. Cutover

Staple should cut over to this markdown-first package model as the primary portability format.

`staple.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Staple, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
