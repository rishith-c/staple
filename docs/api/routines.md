...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Staple-Signature` + `X-Staple-Timestamp` header pair matching the trigger's signing mode.
