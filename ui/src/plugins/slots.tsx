 ...
 *
 * Plugin bundles are built with `external: ["@stapleai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__staplePluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__staplePluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__staplePluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__staplePluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@stapleai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@stapleai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@stapleai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@stapleai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@stapleai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__staplePluginBridge__) {
 ...
 * self-register via `window.staplePlugins.registerReactComponent()`.
 ...
 * of `@stapleai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
