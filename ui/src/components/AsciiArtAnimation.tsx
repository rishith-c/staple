...
type StapleSprite = (typeof STAPLE_SPRITES)[number];
...
function spriteSize(sprite: StapleSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation({ clipCountBoost = 0 }: AsciiArtAnimationProps) {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = STAPLE_SPRITES[(baseCol + baseRow) % STAPLE_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = STAPLE_SPRITES[Math.floor(Math.random() * STAPLE_SPRITES.length)]!;
