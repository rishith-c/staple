...
ClipHub is the public registry where people share, discover, and download Staple company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Staple what a package registry is to a programming language. Staple already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Staple — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Staple instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Staple spec. It contains:
...
**Install (fresh start):**
```
staple install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Staple instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Staple, export your company as a template, then publish:

```
staple export --template my-company
staple publish cliphub my-company
```
...
ClipHub is a **separate service** from Staple itself. Staple is self-hosted; ClipHub is a hosted registry that Staple instances talk to.
...
| **Staple CLI** | `stapleai install`, `stapleai publish`, `stapleai cliphub sync` — built into Staple |
| **Staple UI** | "Browse ClipHub" panel in the Staple web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Staple) |
| Backend | TypeScript + Hono (consistent with Staple) |
| Database | PostgreSQL |
...
4. Run `stapleai install cliphub:acme/lean-saas-shop`
5. Staple creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Staple until it works well
2. Export: `stapleai export --template my-agency`
3. Publish: `stapleai publish cliphub my-agency`
...
3. Install just that agent: `stapleai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Staple

ClipHub is **not required** to use Staple. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Staple what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `stapleai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Staple web UI
- [ ] `stapleai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Staple's own philosophy)
