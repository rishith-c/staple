...
This document defines the rollout plan for adapter-wide skill support in Staple.
...
- Staple stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Staple currently has these adapters:
...
1. Desired skills live in Staple for every adapter.
...
These adapters have a stable local skills directory that Staple can read and manage.
...
These adapters do not have a meaningful Staple-owned persistent install state.
...
- desired skills stored in Staple
...
- runtime already injects Staple skills into `~/.cursor/skills`
...
- runtime already injects Staple skills into `~/.gemini/skills`
...
- runtime already injects Staple skills into `~/.pi/agent/skills`
...
- Staple can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Staple skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Staple may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Staple skills that are clearly marked as Staple-managed
...
- Staple stores desired skills only
...
- avoid destructive wording unless Staple can prove a skill is Staple-managed
...
That gets Staple from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
