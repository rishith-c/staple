...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/staple-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `STAPLE_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `STAPLE_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Staple may create host-local worktrees, processes, and paths
- `cloud`
  - Staple should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `staple | github | vercel | s3 | custom`
...
- use `STAPLE_EXECUTION_TOPOLOGY` as a defaulting guardrail
