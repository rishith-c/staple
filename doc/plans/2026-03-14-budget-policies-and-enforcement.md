...
Staple already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Staple should implement next.

## Product Goals

Staple should let operators:
...
For the next implementation pass, Staple should enforce these defaults:
...
Staple should separate subscription-included usage from billed spend:
...
Staple should have two threshold classes:
...
Staple must know that a project is budget-paused and enforce it.
...
Staple must also block execution before new work starts.
