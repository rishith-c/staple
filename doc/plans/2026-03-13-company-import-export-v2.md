...
This document defines the next-stage plan for Staple company import/export.

The core shift is:

- move from a Staple-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Staple.
...
Staple already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Staple
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `staple.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.staple.yaml` sidecar for high-fidelity Staple-specific details
...
11. adapter-aware skill sync surfaces so Staple can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Staple at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Staple sidecar.
...
Staple must not redefine `SKILL.md`.
...
- Staple-specific fidelity lives in `.staple.yaml`
- Staple may resolve and install `SKILL.md` packages, but it must not require a Staple-only skill format
...
### 5.4 Base Package Vs Staple Extension
...
- base package:
  ...
  - no Staple-only runtime fields by default
- Staple extension:
  - `.staple.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Staple tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`staple.manifest.json` is not part of the future package direction.
...
Staple import/export should support these entity kinds:
...
- Staple can treat “this set of agents came from team package X” as the imported-team model
...
Staple should support source references in package metadata with:
...
Staple should define a new adapter surface area around skills:
...
Planned Staple behavior:

- if an adapter supports read, Staple should show current skills in the UI
- if an adapter supports write, Staple should let the user enable/disable imported skills
- if an adapter supports sync, Staple should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.staple.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.staple.yaml` alongside the base package
...
## 10. Storage Model Inside Staple
...
Staple should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `stapleai company export <company-id> --out <path>`
- `stapleai company import <path-or-url> --dry-run`
- `stapleai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Staple storage, or only referenced at import time?
...
3. Should Staple support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Staple aligned with:
