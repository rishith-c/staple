...
This document defines the product and UI plan for skill management in Staple.
...
- Staple company import/export can include skills as package content
...
3. The UI must always tell the truth about what Staple knows:
   - desired state in Staple
...
Staple should treat skills at two scopes:
...
- created locally inside Staple later
...
7. “Understand what Staple knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Staple-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Staple did not install.

These should be clearly marked:

- external
- not managed by Staple
...
- `desired_only`: Staple wants it, adapter does not show it yet
- `external`: adapter has it but Staple does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Staple only tracks desired state
...
- Staple can still track desired skills
...
- `.staple.yaml` may add Staple-specific fidelity, but should not replace the base shortname association model
...
- `.staple.yaml` may add Staple-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Staple should have a company-scoped skill model or managed package model representing:
...
Staple should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Staple-specific.
...
That gives Staple one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
