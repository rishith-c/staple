...
Agents need API keys to authenticate with Staple. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Staple
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Staple generates a token and passes it directly to the agent
...
`STAPLE_API_URL`, `STAPLE_AGENT_ID`, `STAPLE_COMPANY_ID`. We need to
add a `STAPLE_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Staple CLI
...
```
staple auth login
# Opens browser -> user confirms -> token stored at ~/.staple/credentials
```
...
managed by the Staple server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Staple.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Staple SKILL.md (or a link to it)
   - What information Staple needs from the agent (e.g. webhook URL, adapter
...
5. Staple stores the pending registration.
...
7. On approval, Staple provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Staple issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.staple.ing/skills/staple/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Staple should send you heartbeats",
...
1. Generate a Staple invite link for the company.
...
4. A Staple company member approves the new agent.
5. Staple begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `STAPLE_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `stapleai auth login` for developer-managed remote agents.                                      |
