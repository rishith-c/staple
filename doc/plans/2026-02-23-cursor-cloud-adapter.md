...
This document defines the V1 design for a Staple adapter that integrates with
...
4. synthesized stdout events for Staple UI/CLI
...
3. **Agent callback auth to Staple** uses a bootstrap exchange flow (no long-lived Staple key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Staple endpoints, not full SKILL.md prompt injection.
...
- `staplePublicUrl` (optional override; else `STAPLE_PUBLIC_URL` env)
...
## Staple Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `STAPLE_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.staplePublicUrl`
2. `process.env.STAPLE_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Staple credentials in prompt text.

Flow:

1. Before launch/follow-up, Staple mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Staple validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Staple API calls.
...
1. Prompt includes a compact instruction to fetch skills from Staple.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/staple`
   - `GET /api/skills/staple-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <staplePublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Staple run context.
...
5. `staplePublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Staple skill endpoints |
| Staple auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Staple context.
3. Add richer PR metadata surfacing in Staple UI.
