...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Staple plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Staple.
...
- package: `@stapleai/plugin-kitchen-sink-example`
- plugin id: `staple.kitchen-sink-example` or `staple-kitchen-sink-example`

Recommendation: use `staple-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Staple event such as `issue.created`
...
Recommendation: default to a Staple-local endpoint or a stable public echo endpoint to avoid flaky docs.
