...
Today release smoke testing for published Staple packages is manual and shell-driven:
...
- runs `stapleai@${STAPLEAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@staple.local`
- password: `staple-smoke-password`
...
- the suite passes locally against both:
  - `STAPLEAI_VERSION=canary`
  - `STAPLEAI_VERSION=latest`
...
1. A published `stapleai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `stapleai@latest` Docker install can be smoke-tested by Playwright in CI.
