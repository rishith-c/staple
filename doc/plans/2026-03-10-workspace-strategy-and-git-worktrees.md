...
`PAP-447` asks how Staple should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Staple instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Staple:

- some operators use Staple to manage Staple and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Staple
- Claude and Codex expose different built-in affordances, so Staple should not overfit to one tool

## Core Product Decision

Staple should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Staple
...
- Staple should not assume that every adapter can see or use a host filesystem path directly
...
If Staple makes worktrees agent-first, it will blur:
...
- Staple core resolves and records execution workspace state
...
That gives Staple room to support:
...
- Staple handles workspace runtime services under the hood using project/workspace policy
...
Once Staple is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Staple cannot infer
...
- when issue moves to done, should Staple auto-commit?
- should it auto-push?
...
Staple already has the right foundation for a project-first model.
...
For server-spawned local adapters, Staple already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Staple should mirror directly

Implication:

- for `codex_local`, Staple should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Staple should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Staple should separate:
...
Staple should be able to express intentions such as:
...
These are not specific to Staple. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".staple/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Staple-specific `devServer` flag.
...
Staple should distinguish between:
...
Staple should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Staple should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Staple
...
For runtime services specifically, Staple should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Staple-only dev-server flags
...
3. Inject resolved execution workspace details into `context.stapleWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Staple instance can be reused through env/config injection when appropriate
...
- Staple can decide whether to reuse or start a fresh service deterministically
...
3. Test against Staple-on-Staple workflows first.
...
7. Workspace runtime services are modeled generically, not as Staple-specific dev-server toggles.
