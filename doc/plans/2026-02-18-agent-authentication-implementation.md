...
- New config/env settings:
  - `STAPLE_AGENT_JWT_SECRET`
  - `STAPLE_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `STAPLE_AGENT_JWT_ISSUER` (default: `staple`)
  - `STAPLE_AGENT_JWT_AUDIENCE` (default: `staple-api`)
...
1. In:
   ...
   inject `STAPLE_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `STAPLE_API_KEY`, do not overwrite it.
- Continue injecting:
  - `STAPLE_AGENT_ID`
  - `STAPLE_COMPANY_ID`
  - `STAPLE_API_URL`
...
- Local adapters authenticate without manual `STAPLE_API_KEY` config.
