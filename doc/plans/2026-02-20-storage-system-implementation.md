...
Add a single storage subsystem for Staple that supports:
...
- API serves attachment bytes through authenticated Staple endpoints.
...
- Default local path is under instance root: `~/.staple/instances/<instanceId>/data/storage`.
...
- `stapleai onboard` writes a valid `storage` config block by default.
- `stapleai configure --section storage` can switch between local and s3 modes.
...
- `stapleai doctor` reports actionable storage status.
