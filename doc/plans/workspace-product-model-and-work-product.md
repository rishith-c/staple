...
Staple needs to support two very different but equally valid ways of working:
...
Today, Staple already has the beginnings of this model:
...
- making Staple feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Staple users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Staple deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Staple into a full code review product
...
This object must be recorded explicitly so that Staple can:
...
Staple should treat PRs as a type of work product linked back to:
...
Staple already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Staple must be a first-class deployment mode

Staple cannot assume that it is running on the same machine as the code.

In cloud deployments, Staple may:
...
- `Runtime service`: a process or service Staple owns or tracks for a workspace
...
It also lets cloud-hosted Staple deployments point at codebases and remotes without pretending the Staple host has direct filesystem access.
...
Without an explicit `execution workspace` record, Staple has nowhere stable to attach:
...
Staple needs a single place to show:
...
- Staple-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Staple owning the execution host
...
## 1. Staple host and execution host must be decoupled
...
without the Staple host itself running local git or local preview processes.
...
Staple should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Staple.
...
Staple should keep workspace automation centered on a small set of hooks:
...
Staple should provide the equivalent visibility in the project `Code` surface:
...
Staple should treat execution workspaces as the natural home for:
...
Staple should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Staple should continue treating cleanup policy as part of the core workspace model:
...
Staple should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Staple host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Staple should borrow Worktrunk's good execution patterns while keeping the broader Staple model:
...
That keeps Staple focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Staple deployments remain first-class

That is a better fit for Staple than either extreme:
