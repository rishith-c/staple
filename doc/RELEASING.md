...
# or
npx stapleai@canary onboard --data-dir "$(mktemp -d /tmp/staple-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/stapleai/staple/actions/workflows/release.yml)
...
```bash
gh workflow run release-smoke.yml -f staple_version=canary
gh workflow run release-smoke.yml -f staple_version=latest
```
