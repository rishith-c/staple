...
Example Staple plugin that demonstrates:
...
```bash
pnpm --filter @stapleai/plugin-file-browser-example build
pnpm stapleai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm stapleai plugin uninstall staple-file-browser-example --force
```
...
- Optional: use `staple-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
