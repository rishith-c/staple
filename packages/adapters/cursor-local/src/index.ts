...
Use when:
- You want Staple to run Cursor Agent CLI locally as the agent runtime
...
- Staple auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$staple" and related skills on local runs.
- Staple auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
