...
describe("agent instructions service", () => {
  const originalStapleHome = process.env.STAPLE_HOME;
  const originalStapleInstanceId = process.env.STAPLE_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalStapleHome === undefined) delete process.env.STAPLE_HOME;
    else process.env.STAPLE_HOME = originalStapleHome;
    if (originalStapleInstanceId === undefined) delete process.env.STAPLE_INSTANCE_ID;
    else process.env.STAPLE_INSTANCE_ID = originalStapleInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const stapleHome = await makeTempDir("staple-agent-instructions-home-");
    const externalRoot = await makeTempDir("staple-agent-instructions-external-");
    cleanupDirs.add(stapleHome);
    cleanupDirs.add(externalRoot);
    process.env.STAPLE_HOME = stapleHome;
    process.env.STAPLE_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("staple-agent-instructions-new-external-");
    cleanupDirs.add(stapleHome);
    cleanupDirs.add(externalRoot);
    process.env.STAPLE_HOME = stapleHome;
    process.env.STAPLE_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("staple-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const stapleHome = await makeTempDir("staple-agent-instructions-recover-");
    cleanupDirs.add(stapleHome);
    process.env.STAPLE_HOME = stapleHome;
    process.env.STAPLE_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const stapleHome = await makeTempDir("staple-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("staple-agent-instructions-stale-root-");
    cleanupDirs.add(stapleHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const stapleHome = await makeTempDir("staple-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("staple-agent-instructions-heal-write-stale-");
    cleanupDirs.add(stapleHome);
    cleanupDirs.add(staleRoot);
    process.env.STAPLE_HOME = stapleHome;
    process.env.STAPLE_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const stapleHome = await makeTempDir("staple-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("staple-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(stapleHome);
    cleanupDirs.add(staleRoot);
    process.env.STAPLE_HOME = stapleHome;
    process.env.STAPLE_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const stapleHome = await makeTempDir("staple-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("staple-agent-instructions-partial-root-");
    cleanupDirs.add(stapleHome);
    cleanupDirs.add(staleRoot);
    process.env.STAPLE_HOME = stapleHome;
    process.env.STAPLE_INSTANCE_ID = "test-instance";
