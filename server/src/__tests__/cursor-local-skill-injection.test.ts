...
import { ensureCursorSkillsInjected } from "@stapleai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Staple skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("staple-cursor-skills-src-");
    const skillsHome = await makeTempDir("staple-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "staple");
    await createSkillDir(skillsDir, "staple-create-agent");
    ...
    const injectedA = path.join(skillsHome, "staple");
    const injectedB = path.join(skillsHome, "staple-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "staple")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "staple-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "staple"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "staple-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("staple-cursor-preserve-src-");
    const skillsHome = await makeTempDir("staple-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "staple");
    ...
    expect((await fs.lstat(path.join(skillsHome, "staple-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("staple-cursor-fail-src-");
    const skillsHome = await makeTempDir("staple-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
