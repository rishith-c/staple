...
<head>
    <!-- STAPLE_RUNTIME_BRANDING_START -->
    <!-- STAPLE_RUNTIME_BRANDING_END -->
    <!-- STAPLE_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from STAPLE_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ STAPLE_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ STAPLE_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ STAPLE_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      STAPLE_IN_WORKTREE: "true",
      STAPLE_WORKTREE_NAME: "staple-pr-432",
      STAPLE_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("staple-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="staple-worktree-name"');
    expect(meta).toContain('content="staple-pr-432"');
    expect(meta).toContain('name="staple-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="staple-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="staple-worktree-name"');
  });
});
