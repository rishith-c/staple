...
describe("gemini local skill sync", () => {
  ...
  it("reports configured Staple skills and installs them into the Gemini skills home", async () => {
    const home = await makeTempDir("staple-gemini-skill-sync-");
    ...
    const after = await syncGeminiSkills(ctx, [stapleKey]);
    expect(after.entries.find((entry) => entry.key === stapleKey)?.state).toBe("installed");
    expect((await fs.lstat(path.join(home, ".gemini", "skills", "staple"))).isSymbolicLink()).toBe(true);
  });

  it("keeps required bundled Staple skills installed even when the desired set is emptied", async () => {
    const home = await makeTempDir("staple-gemini-skill-prune-");
    cleanupDirs.add(home);
    ...
    await syncGeminiSkills(configuredCtx, [stapleKey]);
