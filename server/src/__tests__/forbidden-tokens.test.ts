...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "staple", LOGNAME: "staple", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "staple" }),
      },
    );

    expect(tokens).toEqual(["staple", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "staple" }, {
        userInfo: () => ({ username: "staple" }),
      });

      expect(tokens).toEqual(["staple", "custom-token"]);
    } finally {
