 ...
 */
import { pluginManifestV1Schema } from "@stapleai/shared";
import type { StaplePluginManifestV1 } from "@stapleai/shared";
import { PLUGIN_API_VERSION } from "@stapleai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): StaplePluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as StaplePluginManifestV1,
        };
      }
