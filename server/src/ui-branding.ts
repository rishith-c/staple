const FAVICON_BLOCK_START = "<!-- STAPLE_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- STAPLE_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- STAPLE_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- STAPLE_RUNTIME_BRANDING_END -->";
...
function createFaviconDataUrl(background: string, foreground: string): string {
  const svg = [
    ...
    `<path stroke="${foreground}" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 4h12v0a2 2 0 0 1 2 2v10a6 6 0 0 1-6 6h-4a6 6 0 0 1-6-6V6a2 2 0 0 1 2-2z"/>`,
    "</svg>",
  ].join("");
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.STAPLE_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.STAPLE_WORKTREE_NAME) ?? nonEmpty(env.STAPLE_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.STAPLE_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="staple-worktree-enabled" content="true" />',
    `<meta name="staple-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="staple-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="staple-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
