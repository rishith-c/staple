...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@stapleai/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  stapleUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).stapleUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.stapleUpgradeContext = context;
