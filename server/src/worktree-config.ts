...
function resolveWorktreeRuntimeContext(
  ...
  const worktreeName = nonEmpty(env.STAPLE_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.STAPLE_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
